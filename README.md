...
- **工作流编排** — 多步骤、多文章的自动化任务流水线，支持「逐篇处理」和「汇总整合」两种步骤类型
- **流式输出** — SSE 实时流式返回模型回复，工作流执行时实时展示每步进度
- **调试面板** — 查看完整请求体、模型输出、token 用量和日志
- **LaTeX 渲染** — 支持数学公式显示
- **Web 配置** — 在线管理 API Key、模型参数、系统提示词、任务模板和工作流
...
│   ├── workflow_service.py  # 工作流编排与执行引擎
...
### 工作流模式
1. **选择文章** — 在首页勾选一篇或多篇文章
2. **运行工作流** — 点击底部「运行工作流」按钮，选择工作流模板
3. **实时查看** — 自动跳转到执行页面，实时展示每个步骤的进度和结果
4. **查看历史** — 在「工作流」页面查看所有运行历史和结果

### 配置
- 在配置页面管理 API Key、模型参数、系统提示词、任务模板和工作流编排
